...
{
	int j, w;
	char host[2000], path[2000];
	signal(SIGINT, sigintHandler);
	printf("\n");
	char *user = getenv("USER");
	gethostname(host, sizeof(host));
	getcwd(path, sizeof(path));
	for(j = 0; control[j] != '\0' && path[j] != '\0'; j++)
	{
		if(control[j] != path[j])
		{
			break;
	...
	if(path[j] != '\0' || strlen(path) == strlen(control))
	{
		printf("~");
		for(w = j; a[w] != '\0'; w++)
		{
...
int main()
{
	int i = 0, w, e, done = 0, str;
	int in = dup(0);
	int out = dup(1);
	global = 0;
	initial = 0;
	char a[2000], *command, **q, b[2000], host[2000], eof[2000];
	char *user = getenv("USER");
	getcwd(b, sizeof(b));
	strcpy(control, b);
	gethostname(host, sizeof(host));
